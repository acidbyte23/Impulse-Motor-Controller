...
float multiPulseFrequency = 1500.0;
float previousMultiPulseFrequency;
uint32_t pulseWidthTime;
uint32_t pulseCycleTime;
int multiPulseModeEnable = 0;
int multiPulseModeRunning = 0;
int multPulseCounterLow;
int multPulseCounterHigh;
int totalPulsesLow;
int totalPulsesHigh;
uint32_t multiPulseDelay;
...
static void multiPulseControl(void);
...
static void multiPulseMode(void);
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		...
		if(motorEnable == 1){
			...
			if( multiPulseModeEnable == 1 && (multiPulseModeRunning == 1) ){
				multiPulseControl();
			}
			else{
				pulseControl();		
				multiPulseModeRunning = 0;
			}
...
void multiPulseControl(void){
	// handle the highside pulse
	if((highPulseState == 1) && (TIM5->CNT >= (pulseDelay + multiPulseDelay)) && (multPulseCounterHigh <= totalPulsesHigh)){
		incrementalPulseDone = 0; // reset incremental
		highPulseState = 2; // set state int
		multiPulseDelay += pulseCycleTime;
		multPulseCounterHigh++;
		TIM3->ARR = (pulseWidthTime - 1) / 2; // set pulsewidth
		__HAL_TIM_ENABLE(&htim3); // enable one shot pwm timer
	}

	// handle the lowside pulse
	if((lowPulseState == 1) && (TIM5->CNT >= (pulseDelay + halfTime + multiPulseDelay)) && (multPulseCounterLow <= totalPulsesLow)){
		incrementalPulseDone = 0; // reset incremental
		lowPulseState = 2; // set state int
		multiPulseDelay += pulseCycleTime;
		multPulseCounterLow++;
		TIM4->ARR = (pulseWidthTime - 1) / 2; // set pulsewidth
		__HAL_TIM_ENABLE(&htim4); // enable one shot pwm timer
	}
	
	if((multPulseCounterLow >= totalPulsesLow)){
		lowPulseState = 0;
		multPulseCounterLow = 0;
		TIM4->CNT = 0;
		multiPulseDelay = 0;
	}
	if((multPulseCounterHigh >= totalPulsesHigh)){
		highPulseState = 0;
		multPulseCounterHigh = 0;
		TIM3->CNT = 0;
		multiPulseDelay = 0;
	}
...
void motorRunningState(void){
	...
	// reset pulse states
	if((highPulseState == 2) && (multPulseCounterHigh < totalPulsesHigh) && (multiPulseModeEnable == 1)){
		highPulseState = 1;
	}
	else if((highPulseState == 2)){
		highPulseState = 0;
	}
	
	if((lowPulseState == 2) && (multPulseCounterLow < totalPulsesLow) && (multiPulseModeEnable == 1)){
		lowPulseState = 1;
	}
	else if((lowPulseState == 2)){
		lowPulseState = 0;
	}
...
void multiPulseMode(void){
	// pulseWidth
	//multiPulseFrequency
	//totalPulsesLow
	//totalPulsesHigh
	if((multiPulseFrequency != previousMultiPulseFrequency) && ((highPulseState == 0) && (lowPulseState == 0)) || ((highPulseState == 2) && (lowPulseState == 0)) || ((highPulseState == 0) && (lowPulseState == 2))){ // if no pulse is running
		previousMultiPulseFrequency = multiPulseFrequency;
		pulseCycleTime = (uint32_t)(1000000.0 / multiPulseFrequency);
		pulseWidthTime = (pulseCycleTime / 2) - 1;
		TIM3->ARR = pulseWidthTime;
		TIM4->ARR = pulseWidthTime;
		if(pulseWidthTime > 65535){
			pulseWidthTime = 65534;
		}
		if(pulseWidth > pulseCycleTime){
			totalPulsesLow = (pulseWidth / pulseCycleTime);
			totalPulsesHigh = (pulseWidth / pulseCycleTime);
		}
		else{
			totalPulsesLow = 1;
			totalPulsesHigh = 1;
		}
		multiPulseModeRunning = 1;
	}
	else{
		previousMultiPulseFrequency = 0;
	}
}
/* USER CODE END 4 */
